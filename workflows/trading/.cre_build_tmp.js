...
var onHttpTrigger = (runtime2, _payload) => {
  const alphaVaultAbi = parseAbi([
    "function totalAssets() view returns (uint256)",
    "function currentNAV() view returns (uint256)",
    "function getTradeCount() view returns (uint256)"
  ]);
  const vaultAddr = runtime2.config.receiverAddress;
  const evmClient = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["avalanche-testnet-fuji"]);
  ...
  const totalAssetsResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: vaultAddr,
  ...
  const currentNAVResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: vaultAddr,
      data: currentNAVCall
  ...
  const tradeCountResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: vaultAddr,
      data: tradeCountCall
  ...
  const totalAssets = decodeFunctionResult({
    abi: alphaVaultAbi,
    functionName: "totalAssets",
    data: bytesToHex(totalAssetsResult.data)
  });
  const currentNAV = decodeFunctionResult({
    abi: alphaVaultAbi,
    functionName: "currentNAV",
    data: bytesToHex(currentNAVResult.data)
  });
  const tradeCount = decodeFunctionResult({
    abi: alphaVaultAbi,
    functionName: "getTradeCount",
    data: bytesToHex(tradeCountResult.data)
  });
  runtime2.log(`Vault state — totalAssets: ${totalAssets}, NAV: ${currentNAV}, trades: ${tradeCount}`);
  const confidentialHttp = new ClientCapability2;
  const priceResponse = confidentialHttp.sendRequest(runtime2, {
    request: {
  ...
  const priceData = JSON.parse(text(priceResponse));
  runtime2.log(`Prices — ETH: $${priceData.ethereum.usd}, BTC: $${priceData.bitcoin.usd}, AVAX: $${priceData["avalanche-2"].usd}`);
  const claudeApiKey = runtime2.getSecret({ id: "CLAUDE_SERVICE_API_KEY" }).result().value;
...
Return ONLY the JSON object.`;
  ...
  const claudeResponse = confidentialHttp.sendRequest(runtime2, {
    request: {
      url: `${runtime2.config.claudeServiceUrl}/chat`,
      method: "POST",
...
`);
  ...
  runtime2.log(`AI decision — ${decision.action} ${decision.tokenPair}, amount: ${decision.amount}, confidence: ${decision.confidence}`);
  runtime2.log(`Reasoning (TEE-only): ${decision.reasoning}`);
  if (decision.action === "HOLD") {
    runtime2.log("Decision is HOLD — no on-chain trade.");
    return {};
  }
  const actionEnum = decision.action === "BUY" ? 0 : decision.action === "SELL" ? 1 : 2;
  const priceMap = {
  ...
  const encodedPayload = encodeAbiParameters(parseAbiParameters("uint8, string, uint256, uint256, uint256, uint256"), [
    actionEnum,
    decision.tokenPair,
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    ...
    report: report2,
    gasConfig: { gasLimit: String(runtime2.config.gasLimit) }
  }).result();
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    const txHash = writeResult.txHash ? bytesToHex(writeResult.txHash) : "unknown";
    runtime2.log(`Trade recorded on-chain — TX: ${txHash}`);
  } else {
    runtime2.log(`EVM write status: ${writeResult.txStatus} — check on-chain receipt`);
  }
  return {};
};
